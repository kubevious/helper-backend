...
export class ProcessingTracker implements ProcessingTrackerScoper
{
    ...
    private _interval : TimerObject | null = null;
    
    private _timerScheduler : TimerScheduler;

    constructor(logger : ILogger, timerScheduler : TimerScheduler)
    {
        this._logger = logger;
    ...
    debugOutput()
    {
        ...
        for(const x of _.keys(this._values))
        {
            const info = this._values[x];
            info.debugOutput();
        }
